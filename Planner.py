...
def get_bfs_neighbours(cube_map, x, y, standing_z):

    neighbours = []
    current_cube = cube_map.get((x, y, standing_z - 1))
    ...
    if current_cube is not None and current_cube["type"] == "ladder":
        lower_cube = cube_map.get((x, y, standing_z - 2))
        ...
        elif lower_cube is not None and Movement.can_enter_cube(lower_cube):
            neighbours.append({
                "direction": "down",
                "tile": Sense.get_scan_tile_type(cube_map, lower_cube),
                "position": {
                    "x": x,
                    "y": y,
                    "z": lower_cube["z"] + 1,
    ...
    for direction, delta in DIRECTION_TO_VECTOR.items():
        ...
        target_cube = Movement.get_cube_at_xy(cube_map, target_x, target_y, standing_z = standing_z)
        ...
        if target_cube is None:
            if current_cube is not None and current_cube["type"] in {"ledge", "ladder"}:
                fall_target = Movement.get_fall_target(cube_map, target_x, target_y)
                # Handle ledges and tops of ladders as they are tecnically "None"
                if fall_target is not None and Movement.can_enter_cube(fall_target):
                    neighbours.append({
                        "direction": direction,
                        "tile": "safe_fall",
                        "position": {
...
def get_reachable_key_from_position(cube_map, x, y, standing_z):
    for direction, delta in DIRECTION_TO_VECTOR.items():
        ...
        if key_cube is None:
            key_cube = cube_map.get((x + dx, y + dy, standing_z - 1))
...
def analyse_action_with_bfs(cube_map, action, depth_limit=32): # Max distance between checkpoint and relevent objective
    # Action to direction conversion
    direction = MOVE_ACTION_TO_DIRECTION[action]
    dx, dy = DIRECTION_TO_VECTOR[direction]

    start_x = main.agent["x"] + dx
    start_y = main.agent["y"] + dy

    current_cube = cube_map.get((main.agent["x"], main.agent["y"], main.agent["z"] - 1))
    start_cube = Movement.get_cube_at_xy(cube_map, start_x, start_y, standing_z = main.agent["z"])
    start_type = Sense.get_scan_tile_type(cube_map, start_cube)

    if start_cube is None:
        # Also check for ladders
        if current_cube is not None and current_cube["type"] in {"ledge", "ladder"}:
            fall_target = Movement.get_fall_target(cube_map, start_x, start_y)
    ...
    can_start = Movement.can_move_between(current_cube, start_cube)
    ...
    current_position = (main.agent["x"], main.agent["y"], main.agent["z"])
    ...
    while queue:
        ...
        reachable_key = get_reachable_key_from_position(cube_map, x, y, standing_z)
        ...
        neighbours = get_bfs_neighbours(cube_map, x, y, standing_z)
...
def get_bfs_analysis(cube_map):
    ...
    for action in MOVE_ACTION_TO_DIRECTION:
        bfs_analysis[action] = analyse_action_with_bfs(cube_map, action)
